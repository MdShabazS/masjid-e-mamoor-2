...
-- Anonymous Donations V1.
--
-- Anonymous means the donor/member identity is not stored. The authenticated
-- staff operator remains auditable through recorded_by_application_user_id.
...
create or replace function public.create_anonymous_donation(
...
begin
  ...
  if not public.has_application_permission('donations.anonymous.create') then
    raise exception 'missing_permission' using errcode = '42501';
  end if;

  if p_amount_paise is null or p_amount_paise <= 0 then
    raise exception 'invalid_anonymous_donation_amount'
      using errcode = '22023';
  end if;
  ...
  if found then
    if v_existing.operation_type <> 'anonymous_donation_create'
...
revoke all on function public.create_anonymous_donation(
  bigint, text
)
from public, anon, authenticated;

grant execute on function public.create_anonymous_donation(
  bigint, text
)
to authenticated;
